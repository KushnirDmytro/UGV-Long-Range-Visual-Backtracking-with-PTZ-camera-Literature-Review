...
\makeatother
\setcounter{tocdepth}{2} % Includes sections and subsections
...
There are expected "clustes" or "clogs" in the robotics research publications. Not only around the dedicated fields, problems, laboratories which occur in natural way in each field of research.
The non-typical thing are researches streamlined by the strategic governemntal oganizations. The most well known is DARPA. Now there are more of them, like MBZIRC. \todo{Add more later, support with citations.}

Similar clusters are streamed by non-governemntal organizations: technical giants. \todo{describe the case of Kinect. Examples by Intel, OpenCV, Cameras series... }
...
Steps that we do:
- Taks: Select the problem we are dealing with: from the needs to the vision of the solution.
- Requirement: Decompose task into some basic tasks, stages, states. Constrains also comes here as the type of requirements. Decompose single iteration at once, maintain higher level of abstraction.
- Design:
  Here we have to envision the idea of the system that can satisfy those requirements on highest level. In Robotics the reusability is the key, so here we have to narrow the application domain and look for existing solutions.
- Search the literature for the solutions that can satisfy the placed requirements.
- Filter the results.
  - Prioritiza the experiments or stages of implementations.
...
\comment{This is a Larger piece of text for ROS. Other aspects will be detailed in similar fashion + add references}
...
First iteration should include the bare minimum of the requirements. This is the most dangerous part of the project, where we have the most uncertainty.
From the first pilot implementation state we will be able to map the iterations towards the desired system with all requrements satisfied.

Each iteration of the process will satisfy one or more new requirement.

\todo{Place the example of work phases scheme. Example on how we will get to the PTZ camera is best candidate.}
...
\todo{Place this as the SUBsection to the "Secondary goals and limitations" block}
